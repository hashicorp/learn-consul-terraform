...
start_frontend() {
  #Create Systemd Config
sudo cat << EOF > /etc/systemd/system/frontend.service
[Unit]
Description="HashiCups Frontend"
Documentation=https://github.com/hashicorp-demoapp/frontend
Requires=network-online.target
After=network-online.target
...
ExecStartPre=/bin/bash -c 'NEXT_PUBLIC_PUBLIC_API_URL="/" /usr/bin/npm run build'
...
EOF
  ...
  git clone https://github.com/hashicorp-demoapp/frontend
  cd frontend
  npm install
