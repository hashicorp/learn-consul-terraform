...
resource "local_file" "hashicups_cleanup_rendered" {
  filename = "${path.root}/scripts/patch-resources.sh"
  content = templatefile("${path.module}/templates/patch_and_delete.sh.tftpl", {
      resources = local.resources,
      kubeplanfiles = local.kube_files
  })
}
