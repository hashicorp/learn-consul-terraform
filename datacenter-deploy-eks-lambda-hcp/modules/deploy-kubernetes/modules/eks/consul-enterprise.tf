locals {
  hcp_endpoint = split("https://", var.eks_config.hcp_consul_endpoint)[1]
}
...
resource "helm_release" "consul_enterprise" {
  ...
  set {
    ...
    value = var.eks_config.hcp_datacenter
  }
...
resource "kubernetes_secret" "license" {
  ...
  type = "Opaque"
  data = {
    "key" = var.eks_config.consul_ent_license_b64
  }
...
resource "kubernetes_secret" "bootstrap_token" {
  ...
  type = "Opaque"
  data = {
    "token" = var.eks_config.consul_bootstrap_token_b64
  }
  depends_on = [module.eks]
}
...
resource "kubernetes_secret" "consul_ca_cert" {
  ...
  type = "Opaque"
  data = {
    "tls.crt" = var.eks_config.consul_ca_certificate_b64
  }
  depends_on = [module.eks]
}
